...
import { PencilLine, Plus, Search, FileText, Users, Globe, Lock, BookOpen } from "lucide-react"
import { useToast } from "@/hooks/use-toast"
import { 
  useFillInTheBlankDecks, 
  createFillInTheBlankDeck, 
  useDecks 
} from "@/hooks/api/content"
import { useStudents, assignDeck } from "@/hooks/api/students"
...
export default function FillInTheBlanksPage() {
  ...
  const { decks, isLoading, isError, mutate } = useFillInTheBlankDecks()
  ...
  const handleCreateDeck = async () => {
    ...
    try {
      const deckData = {
        ...
        boundVocabularyDeckId: newDeck.boundVocabularyDeckId || undefined,
      }
      await createFillInTheBlankDeck(deckData)
      toast({
  ...
  const boundDecks = decks.filter(d => d.boundVocabularyDeckId)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-2">
          <h1 className="text-3xl font-bold text-slate-900">Fill in the Blank Decks</h1>
          <p className="text-slate-600">Create and manage fill-in-the-blank exercises for your students</p>
        </div>
        <Button onClick={() => setIsCreateDeckOpen(true)} className="bg-orange-600 hover:bg-orange-700">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <p className="text-sm font-medium text-slate-600">Vocabulary-Bound</p>
                <p className="text-2xl font-bold text-slate-900">{boundDecks.length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent>
          {isError ? (
            ...
            <div className="text-center py-12">
              <PencilLine className="h-12 w-12 text-slate-300 mx-auto mb-4" />
              <p className="text-slate-500 mb-4">
                {searchTerm || filterVisibility !== "all" ? "No decks match your filters" : "No fill-in-the-blank decks created yet"}
              </p>
      ...
      <Dialog open={isCreateDeckOpen} onOpenChange={setIsCreateDeckOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Create New Fill-in-the-Blank Deck</DialogTitle>
            <DialogDescription>
              Create a new deck of fill-in-the-blank exercises. You can optionally bind it to a vocabulary deck for enhanced learning analytics.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-slate-500">
                Binding to a vocabulary deck enables auto-matching and enhanced analytics.
              </p>
            </div>
            <div className="flex items-center space-x-2">
              <Switch
