...
class RevealAnswerOperator implements ProgressOperator {
  async execute(
    ...
    if (currentProgress.type !== 'GENERIC_DECK')
    ...
    const newProgress: GenericDeckProgress = {
      ...currentProgress,
...
class SubmitRatingOperator implements ProgressOperator {
  async execute(
    ...
    await services.fsrsService.recordGenericReview(
      services.studentId,
    ...
    const allCardsInSession = await services.tx.studentGenericCardState.findMany({
      where: {
