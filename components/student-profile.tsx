...
import { MoreHorizontal, Play, DollarSign, Edit, Archive, BookOpen, Calendar as CalendarIcon, TrendingUp, Clock } from "lucide-react" // Renamed Calendar to CalendarIcon
import { useToast } from "@/hooks/use-toast"
import { useStudent, useDecks, assignDeck, recordPayment, createSchedule } from "@/hooks/use-api-enhanced"
import { format } from "date-fns"
import { SessionStartDialog } from "@/components/session-start-dialog"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
import { Calendar as CalendarComponent } from "@/components/ui/calendar" // Renamed Calendar to CalendarComponent
...
export function StudentProfile({ studentId }: StudentProfileProps) {
  ...
  const [classesPurchased, setClassesPurchased] = useState<number | string>('')
  ...
  // Create handler for recording payment
  const handleRecordPayment = async () => {
    if (!student || !paymentAmount || !classesPurchased) {
    ...
    try {
      await recordPayment(student.id, {
        ...
        classesPurchased: Number(classesPurchased),
        paymentDate: new Date().toISOString(),
      })
      toast({ title: "Payment recorded successfully!" })
      mutate() // Re-fetch student data to update UI
      setIsPaymentDialogOpen(false)
      setPaymentAmount('')
      setClassesPurchased('')
    } catch (error) {
      toast({ title: "Error recording payment", description: (error as Error).message, variant: "destructive" })
    } finally {
      setIsSubmittingPayment(false)
    }
  }

  // Create handler for scheduling a class
  const handleScheduleClass = async () => {
    if (!student || !scheduleDate) {
      toast({ title: "Error", description: "Please select a date and time.", variant: "destructive" })
      return
    }
    setIsScheduling(true)
    try {
      await createSchedule(student.id, { scheduledTime: scheduleDate.toISOString() })
      toast({ title: "Class scheduled successfully!" })
      mutate() // Re-fetch student data
      setIsScheduleDialogOpen(false)
      setScheduleDate(undefined)
    } catch (error) {
      toast({ title: "Error scheduling class", description: (error as Error).message, variant: "destructive" })
    } finally {
  ...
  const paymentColumns = [
    ...
    },
    {
      key: "amount",
      header: "Amount",
      render: (value: any) => `$${typeof value === 'object' && value.toFixed ? value.toFixed(2) : Number(value).toFixed(2)}`,
    },
    ...
    <div className="space-y-6">
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="classes" type="number" placeholder="0" value={classesPurchased} onChange={(e) => setClassesPurchased(e.target.value)} />
            </div>
            <div className="flex justify-end space-x-2">
              <Button variant="outline" onClick={() => setIsPaymentDialogOpen(false)} disabled={isSubmittingPayment}>
                Cancel
              </Button>
              <Button onClick={handleRecordPayment} disabled={isSubmittingPayment} className="bg-blue-600 hover:bg-blue-700">
      ...
      <Dialog open={isScheduleDialogOpen} onOpenChange={setIsScheduleDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Schedule Class for {student.name}</DialogTitle>
            <DialogDescription>
          ...
          <div className="flex justify-end space-x-2">
            ...
            <Button onClick={handleScheduleClass} disabled={isScheduling || !scheduleDate} className="bg-blue-600 hover:bg-blue-700">
